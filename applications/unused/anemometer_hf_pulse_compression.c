...
#define H 95 // 99 has some problem
#define L 0
...
//#define N L,H
#define BEAM    P, N, P, P //
const uint16_t cpulse[] = {PRE_BEAM, BEAM}; //
...
#define PATTERN_LEN  (400)  // 25 = 1 pulse
float pattern[4][PATTERN_LEN] = {0};
float sig_temp_out[VALID_LEN+PATTERN_LEN+1];
...
void thread_anemometer(void* parameters)
{
    ...
    // hardware power_on
    ane_pwr_control(160*1000, true);

//    int pattern_size = get_pattern_len(pulse_len, 160*1000)*sizeof(float);
//    pattern_template = malloc(pattern_size);
//    memset(pattern_template, 0, pattern_size);
   // create_pattern(cpulse, pulse_len, pattern_template, 160*1000, -1);


    uint16_t beam[] = {BEAM};
    int pattern_size = get_pattern_len(sizeof(beam)/sizeof(uint16_t), 160*1000)*sizeof(float);
    pattern_template = malloc(pattern_size);
    memset(pattern_template, 0, pattern_size);
    create_pattern(beam, sizeof(beam)/sizeof(uint16_t), pattern_template, 160*1000, -1);
