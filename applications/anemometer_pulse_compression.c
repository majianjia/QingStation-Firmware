...
#define PATTERN_LEN  (350)  // 25 = 1 pulse
...
int calibration(float* static_zero_cross, float* echo_shape, float* sig,
        const uint16_t *pulse, const uint16_t pulse_len)
{
    ...
    for (int i = 0; i<100 && count<16; i++)
    {
        ...
        for(int idx = 0; idx < 4; idx++)
        {
            ...
            // test record pattern;
            if(pattern[idx][0] == 0)
                memcpy(pattern[idx], &sig[DEADZONE_OFFSET], sizeof(float)*PATTERN_LEN);

            //correlation(pattern[idx], PATTERN_LEN, &sig[DEADZONE_OFFSET], VALID_LEN, sig_temp_out);
            correlation(pattern_template, 350, &sig[DEADZONE_OFFSET], VALID_LEN, sig_temp_out);

            for(int j=0; j<PATTERN_LEN; j++)
                printf("%f\n", pattern[idx][j]);

            for(int j=0; j<sizeof(sig_temp_out)/4; j++)
                printf("%f\n", sig_temp_out[j]);
...
void thread_anemometer(void* parameters)
{
    ...
    LOG_I("Height %dmm, Pitch:%dmm, ADC DeadZone %d, ADC valid len %d",
...
//    uint16_t modulated_pulse[] = {H, H, L}; //++--++----++
//    int pattern_size = get_pattern_len(pulse_len, 80*1000)*sizeof(float);
//    pattern_template = malloc(pattern_size);
//    memset(pattern_template, 0, pattern_size);
//    create_pattern(modulated_pulse, sizeof(modulated_pulse)/2, pattern_template, 10*1000, -1);
...
//        send_to_processing(1000, 0, ADC_SAMPLE_LEN-50, cpulse, pulse_len);
    ...
    int count = calibration((float*)static_zero_cross, (float*)ref_shape, sig, cpulse, pulse_len);
    if(count < 5 && count !=0)
    ...
    */
    ...
    while(1)
    {
        ...
        for(int idx = 0; idx < 4; idx ++)
        {
            ...
            memcpy(corr_sig[idx], &sig[DEADZONE_OFFSET+100], CORR_LEN*sizeof(float));
            ...
            if(isnanf(mse[0])){
                anemometer.err_code = ERR_MSE_NAN;
            ...
            if(mse[mini_mse] > mse_history[idx]*10)
            {
                ...
                anemometer.err_code = ERR_SHAPE_MISMATCH;
                is_data_correct = false;
            }
        ...
        correlation(corr_sig[NORTH], CORR_LEN, corr_sig[SOUTH], CORR_LEN, corr_sigout);
        ...
        if(270 > c || c > 365)
        {
            anemometer.err_code = ERR_WINDSPEED;
            continue;
        }
