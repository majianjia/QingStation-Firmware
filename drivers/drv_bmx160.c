...
int bmx160_init(struct imu_dev_t * dev)
{
    ...
    imu->bmi.aux_cfg.aux_sensor_enable = 1; // auxiliary sensor enable
    imu->bmi.aux_cfg.aux_i2c_addr = BMI160_AUX_BMM150_I2C_ADDR; // auxiliary sensor address
    imu->bmi.aux_cfg.manual_enable = 1; // setup mode enable
    imu->bmi.aux_cfg.aux_rd_burst_len = 2;// burst read of 2 byte

    /* Configure the BMM150 device structure by
    mapping user_aux_read and user_aux_write */
    imu->bmm.read = bmx_i2c_read;
    imu->bmm.write = bmx_i2c_write;
    ...
    imu->bmm.settings.preset_mode= BMM150_PRESETMODE_LOWPOWER;
    ...
    imu->bmi.aux_cfg.aux_odr = 8; /* Represents polling rate in 100 Hz*/
